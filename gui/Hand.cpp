...
Hand::Hand(sf::Vector2f position, float rotation)
{
    ...
    card_transforms[0].translate(-60, 0);

    card_transforms[1].rotate(rotation, position);
    card_transforms[1].translate(60, 0);
}
...
Card * Hand::pickCard(unsigned short index)
{
    Card * c = cards[index];
    if (index == 0) {
        cards[0] = cards[1];
...
sf::Vector2f Hand::freeZonePosition()
{
    return card_transforms[1].transformPoint(getPosition());
}
...
void Hand::update(sf::RenderWindow &window, float dt)
{
    if (cards[0]) {
        sf::FloatRect rect;
        sf::Vector2f position = card_transforms[0].transformPoint(getPosition()),
                scale = cards[0]->getScale();
    ...
    if (cards[1]) {
        sf::FloatRect rect;
        sf::Vector2f position = card_transforms[1].transformPoint(getPosition()),
                scale = cards[1]->getScale();
